...
class PackageTests(unittest.TestCase):
    def test_dataset_integrity(self) -> None:
        ...
        self.assertEqual(manifest["image_count"], 36)
        self.assertEqual(len(manifest["images"]), 36)
        self.assertEqual(len({item["sha256"] for item in manifest["images"]}), 36)
        for item in manifest["images"]:
            image_path = dataset / item["image"]
            caption_path = dataset / item["caption_file"]
            digest = hashlib.sha256(image_path.read_bytes()).hexdigest()
            self.assertEqual(digest, item["sha256"])
            self.assertTrue(caption_path.read_text(encoding="utf-8").startswith("LQK4N. "))
    ...
    def test_dataset_loads_as_imagefolder(self) -> None:
        ...
        self.assertEqual(dataset.num_rows, 36)
    ...
    def test_checkpoint_and_state_are_one_commit(self) -> None:
        ...
        self.assertIn('primary_patterns.append("littlequeen-flux2-klein-v1.safetensors")', source)
