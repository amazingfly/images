...
SESSION="${COLAB_SESSION:-lq-accessories-v2-isolation}"
GPU="${COLAB_GPU:-T4}"
TEST_ROOT="${ROOT}/outputs/accessories_v2_isolation/colab_fp16_832x1216_20260721"
SUCCESS=0
...
colab upload -s "${SESSION}" "${ROOT}/colab_accessories_v2/remote_test_isolation.py" /content/remote_test_accessories_v2_isolation.py
colab upload -s "${SESSION}" "${ROOT}/config_accessories_v2_isolation_prompts.json" /content/lqaccessories_v2_isolation_prompts.json

colab exec -s "${SESSION}" -f "${ROOT}/colab_accessories_v2/launch_test_isolation.py" --timeout 21600
mkdir -p "${TEST_ROOT}"
COLAB_PYTHON="$(sed -n '1s/^#!//p' "$(command -v colab)")"
"${COLAB_PYTHON}" "${ROOT}/scripts/recover_colab_artifact.py" \
  --session "${SESSION}" /content/lqaccessories_v2_isolation_results.tar.gz "${TEST_ROOT}/results.tar.gz"
rm -rf "${TEST_ROOT}/output" "${TEST_ROOT}/test_summary.json"
tar -xzf "${TEST_ROOT}/results.tar.gz" -C "${TEST_ROOT}"
SUCCESS=1
printf 'Downloaded v2 identity-only isolation grid.\n'
