#!/usr/bin/env python3
"""Prepare the manually curated Moonstar outfit SDXL LoRA dataset."""
...
DEFAULT_MANIFEST = ROOT / "training_dataset_outfit_v1" / "curated_manifest.json"
DEFAULT_SOURCE = (
    ROOT
    / "outputs"
...
def main() -> int:
    ...
    if len(scenes) != 10 or set(scenes.values()) != {3}:
        raise SystemExit(f"expected three images from each of ten scenes: {dict(scenes)}")

    args.output.mkdir(parents=True, exist_ok=True)
    for stale in args.output.glob("lqmoonfit_*.*"):
        stale.unlink()
    ...
    for position, entry in enumerate(selected, start=1):
        ...
        source = args.source / f"image_{source_index:03d}_seed_{99000 + source_index}.png"
        ...
        if (width, height) != (832, 1216):
            raise SystemExit(f"unexpected dimensions for {source.name}: {width}x{height}")
        ...
        stem = f"lqmoonfit_{position:03d}"
        ...
        if not caption.startswith("lqmoonfit, solo, lqxl Little Queen,"):
