#!/usr/bin/env python3
"""Install a pinned kohya trainer and train the Moonstar outfit SDXL LoRA."""
...
ROOT = Path("/content/lqmoonfit_v1")
BUNDLE = Path("/content/lqmoonfit_v1_bundle.tar.gz")
...
def main() -> int:
    ...
    final_lora = OUTPUT / "lqmoonfit_sdxl_v1.safetensors"
    ...
    try:
        ...
        status = "failed"
        try:
            run_logged(cache_command("latents", model), LOGS / "cache_latents.log", cwd=SD_SCRIPTS)
            run_logged(cache_command("text", model), LOGS / "cache_text.log", cwd=SD_SCRIPTS)
            run_logged(training_command(model), LOGS / "training.log", cwd=SD_SCRIPTS)
            status = "complete"
        finally:
            stop.set()
