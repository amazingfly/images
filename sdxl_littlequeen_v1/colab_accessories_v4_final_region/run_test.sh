...
SESSION="${COLAB_SESSION:-lq-accessories-v4-final-region}"
GPU="${COLAB_GPU:-T4}"
BUNDLE="${ROOT}/training_dataset_accessories_v4/colab/lqaccessories_v4_final_region_bundle.tar.gz"
RESULTS="${ROOT}/outputs/accessories_v4_final_region/colab_strength_sweep_20260726"
SUCCESS=0
ALLOCATED=0
...
python "${ROOT}/scripts/build_accessories_v4_final_region_bundle.py"
colab new -s "${SESSION}" --gpu "${GPU}"
ALLOCATED=1
upload_chunked "${BUNDLE}" "lqaccessories_v4_final_region_bundle.tar.gz.part_"
...
colab upload -s "${SESSION}" \
  "${ROOT}/colab_accessories_v4_final_region/remote_test.py" \
  /content/remote_accessories_v4_final_region.py
colab exec -s "${SESSION}" -f "${ROOT}/colab_accessories_v4_final_region/launch.py" --timeout 21600

mkdir -p "${RESULTS}"
COLAB_PYTHON="$(sed -n '1s/^#!//p' "$(command -v colab)")"
"${COLAB_PYTHON}" "${ROOT}/scripts/recover_colab_artifact.py" \
  --session "${SESSION}" \
  /content/lqaccessories_v4_final_region_results.tar.gz \
...
SUCCESS=1
printf 'Downloaded V4 final regional validation to %s\n' "${RESULTS}"
